...
def _collect_trajectories(
    ...
    obss = [torch.as_tensor(np.asarray(o, dtype=np.float32), dtype=torch.float32, device=device) for o in obss]
    ...
    while running.any():
        ...
        next_obss = [torch.as_tensor(np.asarray(o, dtype=np.float32), dtype=torch.float32, device=device) for o in next_obss]
        ...
        if done:
            ...
            if "action_mask" in info:
                ...
                action_mask = torch.as_tensor(
                    np.asarray(info["action_mask"], dtype=np.float32),
            ...
            with torch.no_grad():
                ...
                obss = torch.as_tensor(np.asarray(obss, dtype=np.float32), dtype=torch.float32, device=device).unsqueeze(1)
...
def main(envs, eval_env, logger, time_limit, **cfg):
    ...
    while step < cfg.total_steps + 1: 
        ...
        if cfg.save_interval and (step - last_save) >= cfg.save_interval:
            ...
            print(f"Saved model checkpoint to step {step}.")        
            last_save = step

        if cfg.video_interval and (step - last_video) >= cfg.video_interval:
            print(f"Saved Video {step}.") 
            record_episodes(
                eval_env,
                model,
                cfg.video_frames,
